    ...
    try {
        const ingredientUpdates = new Map<
            ...
            for (const product of args.products) {
                const productDetails = await productsService.findOneProductById({ id: product.productId });
                ...
                for (const productIngredient of productDetails.ingredients) {
                    ...
                    const ingredientData = ingredientUpdates.get(productIngredient.ingredientId)!;
            ...
            await Promise.all(
                Array.from(ingredientUpdates.entries()).map(async ([ingredientId, { updatedStock, consumedQuantity, ingredient }]) => {
                    if (updatedStock < 0) {
                        logger.error(`Stock for ingredient ${ingredientId} is insufficient`);
                        throw new Error(`Stock for ingredient ${ingredientId} is insufficient`);
                    }
