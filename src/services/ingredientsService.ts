...
export class IngredientsService {
    ...
    async findOneIngredientById(query: { id: string }) {
        try{
            const ingredient = await this.ingredientsRepository.findOneIngredientById(query);
            ...
            logger.error(`Error happened while finding ingredient with id: ${query.id}`, error);
            throw error;
        }
    }
    async updateStockByIngredientId(query: { id: string, stock: number }) {
        try{
            return await this.ingredientsRepository.updateStockByIngredientId(query);
        }catch(error){
            logger.error(`Error happened while updating stock for ingredient with id: ${query.id}`, error);
            throw error;
        }
    }
}
